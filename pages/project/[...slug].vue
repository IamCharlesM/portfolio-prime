<template>
  <div>
    ...
    <div
      ...
      :key="index"
      class="mx-auto max-w-7xl flex flex-col text-center gap-3 py-10 px-4"
    >
      <h1
        v-motion-fade-visible-once
        class="text-center text-3xl font-bold lg:text-6xl"
      >
        {{ post.title }}
      </h1>

      <h3 v-motion-fade-visible-once>Last updated {{ formattedDate }}</h3>
      <div class="mx-auto">
        <img
          v-motion-fade-visible-once
        ...
        <div class="flex flex-wrap gap-3 justify-center w-full">
          <span v-for="(tag, index) in post.tags" :key="index" class="">
            <Badge v-motion-slide-visible-once-bottom>{{ tag.name }}</Badge>
          </span>
        </div>
      </div>
      <div
        v-motion-fade-visible-once
        v-html="post.html"
        class="prose text-white lg:prose-lg mx-auto max-w-7xl"
      ></div>
    </div>
  </div>
</template>
...
const { data, pending } = await useFetch(
  `https://content.charlesisa.dev/ghost/api/content/posts/slug/${slug}/?key=364f2b2dc67c09643ffad31928&include=tags`
);
